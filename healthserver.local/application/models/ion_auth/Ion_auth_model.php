...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->load->library('ion_auth/google_authenticator');
		...
		$this->alt_identity_column = $this->config->item('alt_identity', 'ion_auth');
		...
		if ($this->hash_method == 'bcrypt') {
			...
			$this->load->library('ion_auth/bcrypt',$params);
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, password, otp, active, last_login')
						  ->where($this->identity_column, $identity)
						  ->or_where($this->alt_identity_column , $identity)
		                  ->limit(1)
	    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
